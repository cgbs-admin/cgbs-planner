...
function isGottesdienstCategory(cat: Category): boolean {
  return (cat.name || "").toLowerCase().includes("gottesdienst");
}
  ...
  useEffect(() => {
    const loadPlanningLevels = async () => {
      try {
        ...
        // Support both: apiFetch -> Response and apiFetch -> JSON
        let json: any = raw;
        if (raw && typeof (raw as any).json === "function") {
          try {
            json = await (raw as any).json();
          } catch (e) {
            console.error("Error parsing /planning-levels JSON", e);
            json = null;
          }
        }
        ...
        if (Array.isArray(json)) {
          ...
          data = (json as any).items;
        } else if (json && Array.isArray((json as any).results)) {
          data = (json as any).results;
        } else {
          console.error("Unexpected /planning-levels response shape", json);
        }
...
// Load categories to offer event types in step 1
useEffect(() => {
  const loadCategories = async () => {
    try {
      const raw = await apiFetch("/categories");
      console.log("CreateEventWizard /categories response", raw);
      ...
      if (Array.isArray(json)) {
        ...
        console.error("Unexpected /categories response shape", json);
      }
...
const handleSelectCategory = (cat: Category) => {
  ...
  const type: EventType = isGottesdienstCategory(cat) ? "GOTTESDIENST" : "OTHER";
  setEventType(type);
  ...
  const handleSubmitGottesdienst = async () => {
    ...
    try {
      ...
      const categoryIds = selectedCategoryId != null ? [selectedCategoryId] : [];
      ...
      const parentRaw = await apiFetch("/events", {
        method: "POST",
        body: JSON.stringify(parentPayload),
      });

      // Support both: apiFetch -> Response and apiFetch -> JSON
      let parent: CreatedEvent;
      if (parentRaw && typeof (parentRaw as any).json === "function") {
        parent = await (parentRaw as any).json();
      } else {
        parent = parentRaw as CreatedEvent;
      }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* STEP 1 – Choose event type (via categories) */}
{step === 1 && (
  <div className="flex flex-col gap-4">
    <h2 className="text-lg font-semibold">Welchen Event-Typ möchten Sie anlegen?</h2>
    <p className="text-sm text-gray-600">
      Wählen Sie einen Event-Typ aus der Liste. Für{" "}
      <span className="font-medium">Gottesdienst</span> steht Ihnen ein
      optimierter Ablauf mit Startzeiten und Unter-Events zur Verfügung.
    </p>
    ...
    {categories.length > 0 ? (
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {categories.map((cat) => {
          ...
          const isGodi = isGottesdienstCategory(cat);
          ...
          return (
            ...
            >
              <div
                ...
                style={
                  cat.color_hex
                    ? { backgroundColor: cat.color_hex, color: "#ffffff" }
                    : undefined
                }
              >
                {cat.symbol ?? (cat.name ? cat.name.charAt(0) : "?")}
              </div>
      ...
      <div className="flex flex-col gap-3">
        <div className="rounded-md border border-dashed px-3 py-2 text-sm text-gray-600">
          Keine Kategorien geladen. Bitte prüfen Sie, ob Kategorien im System angelegt sind
          und ob der Aufruf von <code>/categories</code> erfolgreich ist.
        </div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <button
            type="button"
...
{step === 2 && eventType === "GOTTESDIENST" && (
  <div className="flex flex-col gap-4">
    ...
    <p className="text-sm text-gray-600">
      Wählen Sie ein Datum im Kalender. Nach dem Klick gelangen Sie direkt zur Auswahl der Startzeiten.
    </p>

    {date && (
      <div className="text-sm font-medium text-gray-700">
        Ausgewähltes Datum: {formatDateGerman(date)}
      </div>
    )}
